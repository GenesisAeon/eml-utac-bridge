...
This package has no runtime dependency on other `GenesisAeon/*` packages.
If one is ever added, pin it with a `>=` lower bound matching the minimum
...
issues tracked in `genesis-os`'s `DEPENDENCY_REPORT.md`.
